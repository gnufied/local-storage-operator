	...
	matcher.Eventually(func() bool {
		...
		for _, lvdl := range lvdlList.Items {
			if ok := pvNameSet.Has(lvdl.Spec.PersistentVolumeName); ok {
				...
				if lvdl.Status.CurrentLinkTarget == "" || lvdl.Status.PreferredLinkTarget == "" || lvdl.Status.SymlinkPath == "" {
					return false
				}
...
func assertLVDLsContainTargetAndNodes(t *testing.T, lvdls []localv1.LocalVolumeDeviceLink, expectedTarget string, expectedNodeNames []string) {
	...
	for _, lvdl := range lvdls {
		...
		matcher.Expect(lvdl.Status.SymlinkPath).ToNot(gomega.BeEmpty(), "expected SymlinkPath for LVDL %q", lvdl.Name)
...
func assertLVDLSymlinkPathMatchesPVs(t *testing.T, lvdls []localv1.LocalVolumeDeviceLink, pvs []corev1.PersistentVolume) {
	...
	for i := range lvdls {
		...
		if pv.Spec.Local != nil && pv.Spec.Local.Path != "" {
			matcher.Expect(lvdl.Status.SymlinkPath).To(gomega.Equal(pv.Spec.Local.Path),
