...
func (c CurrentBlockDeviceInfo) getLVDLAndSymlinkPath(ctx context.Context, client client.Client) (*v1.LocalVolumeDeviceLink, string, error) {
	...
	if lvdl.Status.SymlinkPath != "" {
		klog.V(4).Infof("found symlink path %s from LVDL %s status", lvdl.Status.SymlinkPath, lvdl.Name)
		return lvdl, lvdl.Status.SymlinkPath, nil
	}
